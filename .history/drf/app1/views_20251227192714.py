...
#         def get(self,request,singer):
#             data=Songs.object.filter(singer=singer).values()
#             print(data)
#             serializeddata=SongsSerializer(data,many=True)
#             final_data=serializeddata.data
#             return Response({"status":"success","data":final_data})
...
class Song(APIView):
    ...
    def post(self, request):
        data = SongsSerializer(data=request.data)
        if data.is_valid():
            data.save()
        print(data.data, "Hello it is a Serializing")
        return Response({"status": "success", "data": data.data})


# # ✅ CLASS‑BASED getSongsBySinger (not nested)
# class getSongsBySinger(APIView):
#     def get(self, request, singer):
#         data = Songs.objects.filter(singer=singer).values()
#         print(data)
#         serializeddata = SongsSerializer(data, many=True)
#         final_data = serializeddata.data
#         return Response({"status": "success", "data": final_data})
class getSongsBySinger(APIView):
    def get(self, request, singer):
        qs = Songs.objects.filter(singer=singer).values()
        serialized = SongsSerializer(qs, many=True)
        return Response({
            "status": "success",
...
class Users(APIView):
    ...
    def post(self, request):
        userSrls = UsersSerializer(data=request.data)
        if userSrls.is_valid():
            userSrls.save()
        print(userSrls.data, "Hello it is a Serializing")
        return Response({"status": "success", "data": userSrls.data})
